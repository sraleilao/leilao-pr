    ...
    for uf in estados:
        ...
        for termo in termos:
            try:
                ...
                tem_estado = (estado_lower in texto_pagina or uf.lower() in texto_pagina)
                if not tem_estado:
                    continue
...
def garantir_aba_resultados(sheet):
    try:
        ...
        # Limpa APENAS se for o primeiro bloco
        if ABA_LEILOEIROS == "Pagina1":
            print("Limpando aba Resultados para nova rodada...", flush=True)
            aba.clear()
