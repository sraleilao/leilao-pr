...
PALAVRAS_IMOVEL = [
    ...
    "condominio","condomínio","loteamento"
]

PALAVRAS_NAO_IMOVEL = [
    "veiculo","veículo","automóvel","automovel","caminhao","caminhão",
    "onibus","ônibus","moto ","motocicleta","camionete","pickup",
    "maquina","máquina","equipamento","sucata","eletronico","eletrônico",
    "semovente","animal","gado","cavalo","trator","reboque","trailer",
    "lancha","barco","aeronave","implemento"
]

PALAVRAS_LINK_IGNORAR = ["edital","facebook","twitter","instagram","whatsapp","javascript:","mailto:","#"]
...
def ler_configuracoes(sheet):
    try:
        ...
        return {"Estados":"PR","Lance Minimo":"0","Lance Maximo":"999999999","Desagio Minimo":"0","Dias Ate Leilao":"30","Tipo":"TODOS"}

def normalizar_link(link):
    """Remove parâmetros UTM e barras finais para deduplicação"""
    link = re.sub(r'\?utm_.*', '', link or "")
    link = re.sub(r'&utm_.*', '', link)
    return link.strip("/").lower()

def carregar_links_ja_gravados(sheet):
    """Lê todos os links já na aba Resultados para evitar duplicatas entre blocos"""
    try:
        ...
        for row in todos[1:]:  # pula cabeçalho
            if len(row) >= 7:
                link = row[6]  # coluna G = Link do Anuncio
                if link:
...
# Carregar links já gravados (evita duplicatas entre blocos)
