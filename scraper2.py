...
TERMOS_BUSCA = [
    ...
    "/imoveis/pr", "/lotes?estado=PR", "/busca?estado=PR",
    "/leiloes?estado=PR", "/?s=parana", "/parana", "/pr",
]
...
def buscar_imoveis(url_site):
    ...
    for termo in TERMOS_BUSCA:
        try:
            ...
            if not any(p in texto_pagina for p in PALAVRAS_PR):
                continue
            if not any(p in texto_pagina for p in PALAVRAS_IMOVEL):
                continue
            cards = soup.select(
                ".lote,.imovel,.card,.produto,.item,article,"
                "[class*='lote'],[class*='imovel'],[class*='card'],[class*='produto'],[class*='result']"
            )
            if not cards:
                cards = soup.find_all(["article","li","div"], limit=100)
            for card in cards[:100]:
                texto = card.get_text(" ", strip=True).lower()
                if not any(p in texto for p in PALAVRAS_PR):
                    continue
                if not any(p in texto for p in PALAVRAS_IMOVEL):
                    continue
...
def garantir_aba_resultados(sheet):
    try:
        ...
        aba = sheet.add_worksheet(title=ABA_RESULTADOS, rows=10000, cols=6)
...
def gravar_linha(aba_resultados, resultado):
    ...
    try:
        ...
        print(f"  Erro ao gravar linha: {e}", flush=True)
...
print(f"Aba '{ABA_RESULTADOS}' pronta!", flush=True)

aba_leiloeiros = sheet.worksheet(ABA_LEILOEIROS)
leiloeiros = aba_leiloeiros.get_all_records()
print(f"Processando aba '{ABA_LEILOEIROS}' com {len(leiloeiros)} leiloeiros...", flush=True)
...
for i, row in enumerate(leiloeiros, start=2):
    ...
    if resultados:
        ...
        for resultado in resultados:
            gravar_linha(aba_resultados, resultado)
